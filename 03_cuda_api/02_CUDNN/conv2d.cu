...
// Conv2d kerner for CPU
void cpuConv2d(float* input, float* kernel, float* output, int width, int height, int inChannels, int outChannels, int kernelSize, int batchSize) {
    int halfKernel = kernelSize / 2;
                        ...
                        for (int ky = -halfKernel; ky <= halfKernel; ky++) {
                            for (int kx = -halfKernel; kx <= halfKernel; kx++) {
                                ...
                                if (ix >= 0 && ix < width && iy >= 0 && iy < height) {
                                    ...
                                    int kernelIdx = ((oc * inChannels + ic) * kernelSize + (ky + halfKernel)) * kernelSize + (kx + halfKernel);
                                    sum += input[inputIdx] * kernel[kernelIdx];
                                }
                            }
                        }
                    }
                    int outputIdx = ((b * outChannels + oc) * height + y) * width + x;
                    output[outputIdx] = sum;
                }
            }
        }
...
int main() {
    ...
    for (int i = 0; i < benchmarkRuns; i++) {
        ...
        cpuConv2d(h_input, h_kernel, h_output_cpu, width, height, inChannels, outChannels, kernelSize, batchSize);
        clock_t cpu_end = clock();
        totalTime_cpu += 1000.0f * (cpu_end - cpu_start) / CLOCKS_PER_SEC;
    }
