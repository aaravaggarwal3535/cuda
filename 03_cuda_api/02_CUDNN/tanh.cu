...
// self made tanh kernel
__global__ void selfMadeTanhKernel(float* input, float* output, int size){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        float x = input[idx];
...
int main() {
    ...
    float *h_input, *h_output_naive, *h_output_cudnn, *h_output_cpu, *h_output_self_made;
    ...
    h_output_self_made = (float*)malloc(tensor_size * sizeof(float));
    ...
    float *d_input, *d_output_naive, *d_output_cudnn, *d_output_self_made;
    ...
    CHECK_CUDA(cudaMalloc(&d_output_self_made, tensor_size * sizeof(float)));
    ...
    float self_made_times[num_benchmark];
    ...
    // Benchmark runs for self-made kernel
    for (int i = 0; i < num_benchmark; ++i) {
        CHECK_CUDA(cudaEventRecord(start));
        selfMadeTanhKernel<<<grid, block>>>(d_input, d_output_self_made, tensor_size);
        CHECK_CUDA(cudaEventRecord(stop));
        CHECK_CUDA(cudaEventSynchronize(stop));
        CHECK_CUDA(cudaEventElapsedTime(&self_made_times[i], start, stop));
    }

    // Calculate average times
    float avg_naive_time = 0.0f, avg_cudnn_time = 0.0f, avg_self_made_time = 0.0f;
    for (int i = 0; i < num_benchmark; ++i) {
        avg_naive_time += naive_times[i];
        avg_cudnn_time += cudnn_times[i];
        avg_self_made_time += self_made_times[i];
    }
    avg_naive_time /= num_benchmark;
    avg_cudnn_time /= num_benchmark;
    avg_self_made_time /= num_benchmark;
    ...
    CHECK_CUDA(cudaMemcpy(h_output_self_made, d_output_self_made, tensor_size * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    bool self_made_correct = verifyResults(h_output_cpu, h_output_self_made, tensor_size);

    // Print results
    printf("Tensor size: %d x %d x %d x %d\n", batch_size, channels, height, width);
    printf("Average Naive CUDA kernel time: %.3f ms\n", avg_naive_time);
    printf("Average cuDNN activation time: %.3f ms\n", avg_cudnn_time);
    printf("Average Self-Made CUDA kernel time: %.3f ms\n", avg_self_made_time);
    printf("Speedup (Naive vs cuDNN): %.2fx\n", avg_naive_time / avg_cudnn_time);
    printf("Speedup (Naive vs Self-Made): %.2fx\n", avg_naive_time / avg_self_made_time);
    printf("Naive kernel results correct: %s\n", naive_correct ? "Yes" : "No");
    printf("cuDNN results correct: %s\n", cudnn_correct ? "Yes" : "No");
    printf("Self-Made kernel results correct: %s\n", self_made_correct ? "Yes" : "No");

    // Clean up
    CHECK_CUDA(cudaFree(d_input));
    CHECK_CUDA(cudaFree(d_output_naive));
    CHECK_CUDA(cudaFree(d_output_cudnn));
    CHECK_CUDA(cudaFree(d_output_self_made));
