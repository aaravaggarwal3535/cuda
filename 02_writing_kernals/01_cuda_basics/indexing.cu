...
__global__ void whoami(void) {
    ...
    int total = gridDim.x * gridDim.y * gridDim.z * blockDim.x * blockDim.y * blockDim.z; // total people in the entire apartment complex

    printf("%04d | %04d | Block(%d %d %d) = %3d | Thread(%d %d %d) = %3d\n",
        total,
        id,
